...
# 预测标签路径
infer_path = r'E:/BME/nnUNetTrainer__nnUNetPlans__3d_lowres/fold_2/validation'
...
for data in infer_list:
    if not data.endswith('.nii.gz'):
        continue  # 忽略非nii.gz格式的文件
    pr_path = os.path.join(infer_path, data)
    gt_data = data.replace("_predict.nii.gz", "_label.nii.gz")  # 根据预测结果的文件名找到对应的金标准文件名
    gt_path = os.path.join(root_path, gt_data)
    ...
    try:
        ...
        sum_result_HD += tmp #binary.hd95(target, predict)
        print('Single case---> Dice:{} HD95:{}'.format(cal_dice(target, predict), [tmp]))
    except:
    ...
    #######################################################################################
    #######################################################################################
sum_result_HD = sum_result_HD / count
sum_result_dice = sum_result / count

print('*' * 50)
print("Average dice score:", sum_result_dice)
print('Mean HD95:', sum_result_HD)
